...
REAL randn_notrig(REAL mu, REAL sigma) 
{
   static bool deviateAvailable=false;	//	flag
   static REAL storedDeviate;			//	deviate from previous calculation
   REAL polar, rsquared, var1, var2;
   ...
   if (!deviateAvailable) {
      ...
      do {
         var1=2.0*( REAL(rand())/REAL(RAND_MAX) ) - 1.0;
         var2=2.0*( REAL(rand())/REAL(RAND_MAX) ) - 1.0;
...
REAL randn_trig(REAL mu=0.0, REAL sigma=1.0) {
   static bool deviateAvailable=false;	//	flag
   static REAL storedDeviate;			//	deviate from previous calculation
   REAL dist, angle;
   ...
   if (!deviateAvailable) {
      ...
      dist=sqrt( -2.0 * log(REAL(rand()) / REAL(RAND_MAX)) );
      angle=2.0 * PI * (REAL(rand()) / REAL(RAND_MAX));
